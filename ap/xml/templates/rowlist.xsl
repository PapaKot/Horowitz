...
function onDel(){return confirm('Confirm the deletion');};
function onMove(a){var p=parseInt(prompt('Enter the new position'));if(p){if(isNaN(p)||p&lt;=0)alert('The position should be an integer greater than zero');else{a.href=a.href+'&amp;pos='+p;return true}};return false;};
function onSwitch(a){
	if(!a._ajaxFlag){
		a._ajaxFlag=true;
...
function onMultiDel(e){
	...
	<form id="{$id}" action="{@uri}#{$id}" method="post">
		...
		<xsl:if test="@add">
			<input type="submit" value="Add" class="add" onclick="this.form.action.value='new';"/>
		</xsl:if>
		<xsl:if test="@delete">
			<input type="button" value="Remove selected" class="del" onclick="onMultiDel(this)"/>
		</xsl:if>
	</form>
</xsl:template>
