...
<xsl:template match="/page/section/form//field[contains(@check,'empty')]" mode="fieldcheck">if(!this['<xsl:value-of select="@name"/>'].value){alert(' Field "<xsl:value-of select="@label"/>" to be filled.');return false;};</xsl:template>
<xsl:template match="/page/section/form//field[contains(@check,'email')]" mode="fieldcheck">if(<xsl:if test="not(contains(@check,'empty'))">this['<xsl:value-of select="@name"/>'].value <xsl:text disable-output-escaping="yes">&amp;&amp;</xsl:text> </xsl:if>!p.test(this['<xsl:value-of select="@name"/>'].value)){alert('In the field  "<xsl:value-of select="@label"/>" entered the wrong e-mail address');return false;};</xsl:template>
<xsl:template match="/page/section/form//field[contains(@check,'num')]" mode="fieldcheck">if(<xsl:if test="not(contains(@check,'empty'))">this['<xsl:value-of select="@name"/>'].value <xsl:text disable-output-escaping="yes">&amp;&amp;</xsl:text> </xsl:if>isNaN(parseFloat(this['<xsl:value-of select="@name"/>'].value.replace(',','.')))){alert('In the field "<xsl:value-of select="@label"/>" you can enter only numbers');return false;};</xsl:template>
...
<xsl:template match="/page/section/form//field[@type='image']">
	...
	<div class="field file">
		<label for="{@name}"><xsl:call-template name="required"/><xsl:value-of select="@label"/></label>
		<input type="button" class="add" value="Add image" id="editimage{$fieldName}"/>
	...
	<div class="fieldset gallery" id="fieldset_{$fieldName}" style="display:none;">
		<h4 class="legend">Photo management</h4>
	</div>
	<script type="text/javascript">
...
<xsl:template match="/page/section/form//field[@type='file']">
	...
	<div class="field file">
		...
		<input type="button" class="add" value="Add file" id="editfile{$fieldId}"/>
...
function </xsl:text><xsl:value-of select="$fieldId"/><xsl:text>SetFileInfo(xml){
	...
	if(ns[0])ns[0].onclick=function(){if(confirm('Confirm the deletion'))</xsl:text><xsl:value-of select="$fieldId"/><xsl:text>SetFileInfo();};
