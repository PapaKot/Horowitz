...
/* Загрузчик файлов */
	var uploader=new qq.FileUploader({
		...
		debug:true,
		template:'<div class="qq-uploader"><div class="qq-upload-drop-area"><span>Drop files here</span></div><div class="qq-upload-button">Upload files</div><ul class="qq-upload-list"></ul></div>',
		fileTemplate:'<li><span class="qq-upload-file"></span><span class="qq-upload-spinner"></span><span class="qq-upload-size"></span><a class="qq-upload-cancel" href="#">Cancel</a><span class="qq-upload-failed-text">Not uploaded</span></li>',
...
/* Кнопка создать папку */
	todo.get('button_create_folder').onclick=function(){
		var folderName=window.prompt('Enter folder name\n(latin characters and numbers)','folder').replace(/(^\s+)|(\s+$)/g,'');
...
/* Кнопка удалить */
	todo.get('button_delete').onclick=function(){
		if(this.form._getSelectedValues().length){
			if(confirm('Please, confirm file deleting.')){
...
/* Кнопка переименовать */
	todo.get('button_rename').onclick=function(){
		...
		}else if(v.length>1)alert('Select one item');
		else alert('Nothing selected');
	};
...
<form action="<?=$_SERVER['PHP_SELF']?>" id="files" method="post">
	...
	<div id="path">Path: <strong>/<?=$_REQUEST['type']?>/<?=$dir->getManagerPath()?></strong></div>
	<div id="fileList">
		<table>
			<col class="name">
			<col class="date">
			...
			<col class="checkbox">
			<tr>
				<th scope="col" style="padding-left:25px;">Name</th>
...
?>
	...
	<div id="log">
		<div class="buttons">
			<input id="button_create_folder" type="button" value="Create folder" class="add">
			<input id="button_rename" type="button" value="Rename" class="disabled" disabled>
			<input id="button_delete" type="button" value="Delete" class="del disabled" disabled>
			<input id="button_cancel" type="button" value="Cancel" class="cancel">
		</div>
		<div id="uploader"></div>
	</div>
</form>
