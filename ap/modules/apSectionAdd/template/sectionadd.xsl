...
<xsl:template match="/page/section/form//field[contains(@check,'new_id')]" mode="fieldcheck">
	if(this['<xsl:value-of select="@name"/>'].value == ''){
		alert('<xsl:text lang="lang">ID required!</xsl:text>');return false;
	};
	if(!this['<xsl:value-of select="@name"/>']._chars){
		if(this['<xsl:value-of select="@name"/>'].value.length &lt; 3){
			alert('<xsl:text lang="lang">ID must contain min 3 chars!</xsl:text>');return false;
		};
		alert('<xsl:text lang="lang">Invalid ID!</xsl:text>');return false;
	};
	if(!this['<xsl:value-of select="@name"/>']._new_id){
		alert('<xsl:text lang="lang">ID already exists!</xsl:text>');return false;
	};
</xsl:template>
...
<xsl:template match="/page/section/form//field[contains(@check,'new_url')]" mode="fieldcheck">
	if(!this['<xsl:value-of select="@name"/>']._chars){
		if(this['<xsl:value-of select="@name"/>'].value.length &lt; 2){
			alert('<xsl:text lang="lang">URL must contain min 2 chars!</xsl:text>');return false;
		};
		alert('<xsl:text lang="lang">Invalid URL!</xsl:text>');return false;
	};
	if(this['<xsl:value-of select="@name"/>'].value &amp;&amp; !this['<xsl:value-of select="@name"/>']._new_url){
		alert('<xsl:text lang="lang">URL already exists!</xsl:text>');return false;
	};
</xsl:template>
	...
	var sel_parent = todo.get('parent'),
		...
		seatch = function(){
			...
			todo.ajax('',function(text,xml){
				...
				for(var i=0;i&lt;ns.length;i++)sel_child.appendChild(new Option('</xsl:text><xsl:text lang="lang">Before</xsl:text><xsl:text disable-output-escaping="yes"> '+ns[i].getAttribute('title'),ns[i].getAttribute('id')));
				sel_child.appendChild(new Option('</xsl:text><xsl:text lang="lang">Last</xsl:text><xsl:text disable-output-escaping="yes">','',true,true));
			},{'id':'</xsl:text><xsl:value-of select="$_sec/@id"/><xsl:text disable-output-escaping="yes">',
				'md':'</xsl:text><xsl:value-of select="/page/section/@module"/><xsl:text disable-output-escaping="yes">',
				'action':'ajax',
