 ...
 *	Предоставление Settings выбора почтовых шаблонов
 *  модуль связан с классом форм и добавлена возможность редактирования Fieldй форм
	...
	foreach($res as $field){
		...
		switch($field->getAttribute('type')){
			...
			case 'password':
				...
				if(isset($pswd) && ($field->getAttribute('name') == 'password-check') && ($pswd != $val)) $this->err('The entered passwords do not match');
			...
			case 'checkbox':
			case 'radio':
				if(!$val) $this->err('Field "'.$field->getAttribute('label').'" not observed');
				break;
			default:
				$mysql = new mysql();
...
function sendEmail($xml,$form){
	...
	if(($xml->query('./field',$xml->de())->item(0) || $xml->query('./attach',$xml->de())->item(0))
		...
		&& ($subject = $form->getAttribute('theme')?$form->getAttribute('theme'):'New message from user site - '.$domain)
