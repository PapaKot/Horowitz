...
function getMessage(){
	...
	switch($_SESSION['apMess'][$this->getMessSessionName()]){
		case 'add_ok':
			$mess = 'Field successfully added'; break;
		case 'add_fail':
			$mess = 'Form fields have incorrect values'; break;
		case 'save_ok':
			$mess = 'Data successfully saved'; break;
		case 'delete_ok':
			$mess = 'Field successfully removed'; break;
		case 'delete_fail':
			$mess = 'An error occurred while field deleting'; break;
		case 'move_ok':
			$mess = 'Field successfully moved'; break;
		case 'move_fail':
			$mess = 'An error occurred while field moving'; break;
		case 'update_ok':
			$mess = 'Field successfully updated.'; break;
		case 'update_fail':
			$mess = 'An error occurred while field updating'; break;
	}
...
function fixedFields($form){
	...
	if($clientSection->getXML()->evaluate('string(./form[1]/@dbSave)',$md)
		...
		&& ($ff = $form->appendField(formSelect::create('uri', 'Database field name:', '/@uri')))
	) {
		$ff->addOption('','...');
