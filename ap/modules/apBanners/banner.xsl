...
<xsl:template match="/page/section/form//field[@type='banner']">
	...
	<div class="field file">
		...
		<input type="button" class="add" value="Choose banner" id="editimage{$fieldName}"/>
	...
	<div class="fieldset gallery" id="fieldset_{$fieldName}" style="display:none;">
		<h4 class="legend">Banner management</h4>
		<div id="banner{$fieldName}"></div>
		<input type="button" value="Remove banner" class="delete" id="bannerDeleteButton{$fieldName}" />
	</div>
...
deleteButton.onclick=function(){
	if(confirm('Please, confirm banner removing'))initBanner();
};
