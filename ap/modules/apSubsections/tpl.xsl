...
<xsl:template match="/page/section/form//field[contains(@check,'sectionId')]" mode="fieldcheck" priority="10">if(!this['<xsl:value-of select="@name"/>'].value.match(/^[a-z]{1}[a-z0-9_-]{2,50}$/i)){alert('The field "<xsl:value-of select="@label"/>" must contain at least three Latin characters\n lowercase, no spaces and should not be the same as the other links to other sections');return false;};</xsl:template>
