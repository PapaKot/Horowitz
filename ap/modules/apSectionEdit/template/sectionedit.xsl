		...
		seatch=function(id){
			...
			todo.ajax('',function(text,xml){
				...
				for(var i=0;i&lt;ns.length;i++){
					if(now_id!=ns[i].getAttribute('id'))sel_child.appendChild(new Option('BEFORE '+ns[i].getAttribute('title'),ns[i].getAttribute('id'),id.toString()==sel,id.toString()==sel));
					sel=ns[i].getAttribute('id');
				};
				option=sel_child.appendChild(todo.create('option',{'value':''},'LAST'));
