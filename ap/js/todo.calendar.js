...
todo.calendar = function(c,settings){
	var d = document,
		options = {
			...
			,dn				: settings&&settings.days			? settings.days				: ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa']
			,mm				: settings&&settings.months			? settings.months			: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'June', 'July', 'Aug', 'Sept', 'Oct', 'Nov', 'Dec']
			,MM				: settings&&settings.monthsFull		? settings.monthsFull		: ['January','February','March','April','May','June','July','August','September','October','November','December']
			,nav			: settings&&settings.nav			? settings.nav				: true
			,navPosition	: settings&&settings.navPosition	? settings.navPosition		: 'before'
			,navTextNext	: settings&&settings.navTextNext	? settings.navTextNext		: 'Next month'
			,navTextPrev	: settings&&settings.navTextPrev	? settings.navTextPrev		: 'Previous month'
			...
			,yearOffsetUp	: settings&&settings.yearOffsetUp	? settings.yearOffsetUp		: 2
			/**constant*/
			,hover			: 'hover'
			,year			: new Date().getFullYear()
			,weekendColor	: 'red'
			,correctDn		: function(d){return d;}
			,makeYears		: function(y){var years = [];for(var i = y - this.yearOffsetDown; i < y + this.yearOffsetUp + 1; i ++)years[i] = i;return years;}
			,makeDate		: function(s){
				s = s.split(/\D+/);return new Date(s[2], s[0] - 1, s[1]);
			}
	...
	if(!c) throw new Exception('Calendar element not found');
		...
		c._calendar = {
			...
			,'date': function() {var parseDate = c.value.split(options.sep); return new Date(parseDate[2],parseDate[0]-1,parseDate[1]);}()
			,'getEvent':function(d){
			...
			,'makeHtmlByDate':function(dd){
				...
				try{
					for (var i = 1,cN = [], title; i <  arr.length; i++,cN = []) {
						...
						title = this.getEvent(dd.getFullYear() + options.sep + (dd.getMonth() + 1) + options.sep + i) || '';
					...
					return 'an error occured while calendar creating'
				}
			}
			...
			,'show': function(e, dd){
				...
				if (this.timer){ResetTimeout(o.timer);o.timer = null;};
