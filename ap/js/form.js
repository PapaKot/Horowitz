...
fs._initImages=function(){todo.loop(this.getElementsByTagName('input'),function(){
	if(this.className=='deleteImageButton')this.onclick=function(){if(window.confirm('Please, confirm image removing.')){var fg=this.parentNode;fg.parentNode._deleteImage(fg);}}
	else if(this.className=='titleImageButton')this.onclick=function(){
		var e=this;
		while(e=e.nextSibling)if(e.className=='titleImageField'){
			var v=window.prompt('Enter your image title',e.value);
			if(v!==null)e.value=v;
			return;
		}
	}
